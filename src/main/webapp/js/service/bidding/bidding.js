/**
 * <pre>
 * 1. 개요 : 공통 처리
 * 2. 처리내용 : 공통사항을 처리하기 위한 스크립트
 * </pre>
 * 
 * 
 * @File Name businessManagement.js
 * @Author : creme55
 * @Date : 2016. 10. 18.
 * @Version : 1.0
 *
 * 
 */

function biddingLeftMenuAct(code){
		if(code==0){
			location.href = "/bidding/powerBid.do";			
		}else if(code==1){
			location.href = "/bidding/recBid.do";
		}
		
}
function powerBidBodyAction(code){
	if(code==0){
		$('#powerBidBody').load('./powerBidSub1.do');
		$('#tab').attr("class","bodytab_list m1");
	}else if(code==1){
		$('#powerBidBody').load('./powerBidSub2.do');
		$('#tab').attr("class","bodytab_list m2");
	}
	
}
function shBid(obj){
	var url;
	switch (obj) {
	case '1':
		url='/bidding/powerBidSub1.json'
		break;
	case '2':
		url='/bidding/powerBidSub2.json'
		break;
	case '3':
		url='/bidding/recBid.json'
		break;

	default:
		url='/bidding/powerBidSub1.json'
		break;
	}
	$.ajax({
		url : url
		,type : 'POST'
		,dataType : 'json'
		,data : $("#bd_form").serialize()
		,success : function(data) {
			$("#list").clearGridData();
			if(data.results!=''){
				var arr =  JSON.parse(data.results);
				jQuery("#list")
			    .jqGrid('setGridParam',
			        { 
			            datatype: 'local',
			            data:arr
			        })
			    .trigger("reloadGrid");
			}
		}
		,fail : function(request, status, error) {
			$.jQueryMsgAlert("code : [" + request.status + "]\n / " + "message : [" + request.responseText + "]\n / " + "error : " + error + "]");
			loading_end();
		}
	});
}
fnMap.bidding.addPopUp = function (title,code){
	$('#popContext').load(fnMap.bidding.getUri(code)+'?code=add',function(){
		popUpSet('popUpDialog', 'open',title+' 등록');
		
		var colNameArr = ["판매희망일", "집합자원명","전체입찰량","입찰시간"];
		for(var num=0;num<24;num++){colNameArr[num+4]=num+1+"h";}
		var colModelArr = [{ name: "판매희망일", width:100, frozen: true},
		                   { name: "집합지원명", width:100, frozen: true},
		                   { name: "전체입찰량", width:100, frozen: true},
		                   { name: "입찰시간", width:100, frozen: true}];
		for(var num=0;num<24;num++){colModelArr.push({ name: num+1+"h",width:50});}		
		setJqGridFormKPX('popList', 'popListPager', colNameArr, colModelArr, '',830,'auto');
		setDatePickerOne('shDate'); 
	    $("#popList").jqGrid("setFrozenColumns");  
	    $("#popList").parents('div.ui-jqgrid-bdiv').css("min-height","30px");
	    $("#popList").parents('div.ui-jqgrid-bdiv').css("padding-bottom","2px");
	});
};
fnMap.bidding.modifyPopUp = function(title,code){
	$('#popContext').load(fnMap.bidding.getUri(code)+'?code=modify',function(){
		popUpSet('popUpDialog', 'open',title+' 수정');
	});
};
fnMap.bidding.getUri = function(code){
	var result="";
	switch (code) {
	case 0:
		result='./powerBidAddNModify.do';
		break;
	case 1:
		result='./recBidAddNModify.do';
		break;
	default:
		break;
	}
	return result;
};
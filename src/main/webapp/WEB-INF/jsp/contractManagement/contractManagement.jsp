...
<table class="tb2" style="width: 100%;height: 100%;float: right;">
	...
	<tbody>
		<c:forEach var="result" items="${results}" varStatus="status">
			<tr>
				<td>${status.count}</td>
				<td>${result.mdt_cntr_no}</td>
				<td>${result.cntr_terms}</td>
				<td>${result.cntr_wnts_dt}</td>
				<td>${result.cntr_wnte_dt}</td>
				<td>${result.enpr_nm}</td>
				<td>${result.mdt_nm}</td>
				<td>${result.rsrs_nm}</td>
				<td>
					<a class="btn_big" href="javascript:fnMap.cM.modifyPopUp('중개계약',0,'${result.seq_no}')">
